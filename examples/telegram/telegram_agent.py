...
logseq = LSPluginUser(name="Telegram", description="Telegram for interstitial note-taking.")
...
async def send_via_telegram(sid):
    ...
    await logseq.Editor.insertBlock(block.uuid, f"Hello, {logseq.settings.admin}!", sibling=False)
