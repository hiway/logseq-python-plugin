...
def schema(self):
    ...
    for key, value in sett.items():
        meta = dict(getattr(self, "__dataclass_fields__")[key].metadata)
        py_type = getattr(self, "__dataclass_fields__")[key].type
        try:
        ...
        meta.update({"value": value, "key": key, "title": key.title(), "type": js_type})
        sett[key] = meta
